...
public final class DiContainerBuilder {
  ...
  private <T> DiContainerBuilder addDependency(
    ...
    var descriptor =
        new DependencyDescriptor<T>(abstractionType, implementationType, lifecycle, generator);
    dependencyDescriptors.put(abstractionType, descriptor);
    return this;
  }

  public <T> DiContainerBuilder addTransient(
      Class<T> abstractionType, Class<?> implementationType) {
  ...
  public <T> DiContainerBuilder addTransient(
      Class<T> abstractionType, Function<AbstractDiContainer, T> generator) {
  ...
  public <T> DiContainerBuilder addScoped(
      Class<T> abstractionType, Function<AbstractDiContainer, T> generator) {
    return addDependency(abstractionType, null, Lifecycle.SCOPED, generator);
  }

  public <T> DiContainerBuilder addSingleton(
      Class<T> abstractionType, Class<?> implementationType) {
